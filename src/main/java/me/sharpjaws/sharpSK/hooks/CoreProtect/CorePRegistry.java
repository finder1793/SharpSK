...
public class CorePRegistry {
	...
	public static void registerCoreP(){
		...
		Skript.registerEffect(EffCoreLog.class, "[(coreprotect|cp)] log block [(0¦removal|1¦placement|2¦interaction)] at %location% (for|by) %offlineplayer%");
		Skript.registerExpression(ExprCoreLookupData.class, ItemStack.class, ExpressionType.SIMPLE, "[(coreprotect|cp)] [(0¦removal|1¦placement|2¦interaction)] lookup data [from by %-offlineplayers%] from %location% in radius %number% from %timespan% ago [excluding %-itemstacks%]");
	}
