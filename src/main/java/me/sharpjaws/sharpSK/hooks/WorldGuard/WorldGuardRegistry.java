	...
	public static void registerwguard(){
		...
		if (Bukkit.getPluginManager().getPlugin("PirateSK") == null){
		 Skript.registerEffect(EffCreateRegion.class, new String[] { "[sharpsk] create wg region %string% between %location% and %location% in [world] %world%" });
	       ...
	       Skript.registerExpression(ExprRegionAt.class, String.class, ExpressionType.SIMPLE, new String[] { "[sharpsk] wg region at %location%" });
	       Skript.registerExpression(ExprAllRegionsInWorld.class, String.class, ExpressionType.SIMPLE, new String[] { "[sharpsk] [all] wg regions in %world%" });
	       Skript.registerEffect(EffSetBlocks.class, new String[] { "[sharpsk] (set [all] blocks in|fill) wg region %string% in [world] %world% (to|with) %itemstack%" });
	     //---------------------
			}
		}
